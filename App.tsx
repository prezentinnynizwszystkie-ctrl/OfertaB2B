...
const Slide = ({ children, id, className = "", nextSection }: { children?: React.ReactNode, id: string, className?: string, nextSection?: string }) => {
  ...
  return (
    <section 
      id={id} 
      // ZMIANA: md:h-[100vh] i md:snap-start dla Desktop. Mobile ma h-auto (normal flow).
      // ZMIANA: Zmniejszono padding mobilny z pt-4 na pt-2 (bardzo ciasno), aby zredukować przerwę pod Hero.
...
function App() {
  ...
  const [centerName, setCenterName] = useState(''); // Stan dla nazwy centrum rozrywki
    ...
    <div className="h-screen w-full bg-slate-50 text-slate-800 selection:bg-blue-900 selection:text-white overflow-hidden relative">
      ...
      {currentView === 'home' ? (
        <>
          ...
          <div className="fixed right-4 md:right-8 top-1/2 -translate-y-1/2 z-50 flex flex-col space-y-4">
